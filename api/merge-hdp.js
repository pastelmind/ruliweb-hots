...
function parseAllSkillsData(heroData, heroId) {
  ...
  const skillDataObject = skillDataArray
    .filter((skillData) => {
...
function parseAllTalentsData(heroData, heroId) {
  ...
  for (const talentLevelPropertyName of Object.keys(heroData.talents)) {
    ...
    talents[talentLevel] = heroData.talents[talentLevelPropertyName].map(
      parseTalentData
    );
  }
...
function relocateSubAbilitiesAfterTalent(hero, talentId, ...subAbilityIds) {
  for (const talentArray of Object.values(hero.talents)) {
    const talentIndex = talentArray.findIndex(
      (talent) => talent.id === talentId
    );
    if (talentIndex !== -1) {
      const subAbilities = subAbilityIds
        .map((id) => {
          ...
          warn(`Cannot find SubAbility ID: ${id}`);
        })
...
function extractSkillTalentInfo(skillTalentData) {
  ...
  if (skillTalentData.nameId) {
    ...
    warn(`Missing nameId field in ${util.inspect(skillTalentData)}`);
  }

  // Extract name
  if (skillTalentData.name) {
    skillTalentInfo.name = new KoEnString(skillTalentData.name);
  } else warn(`${skillTalentData.nameId} is missing a name`);

  // Extract icon
  if (skillTalentData.icon) {
    skillTalentInfo.icon = extractIconId(skillTalentData.icon);
  } else warn(`${skillTalentData.nameId} is missing an icon`);

  // Extract description
  if (skillTalentData.fullTooltip) {
    skillTalentInfo.description = parseTooltip(skillTalentData.fullTooltip);
  } else warn(`${skillTalentData.name} is missing a full tooltip`);

  // Extract short description
  if (skillTalentData.shortTooltip) {
    skillTalentInfo.shortDescription = parseShortDescription(
      skillTalentData.shortTooltip
    );
  }
    ...
    tooltip
      ...
      .replace(
        ...
        (tag, attrs, text) => {
          ...
          if (
            /AbilityPassive|00ff90/gi.test(color) &&
    ...
    if (resourceMatch) {
      ...
      if (resourceName === "마나") {
        ...
        extras[resourceName] = perSecondString
          ? `${perSecondString} ${resourceCostAmount}`
          : resourceCostAmount;
      }
    } else {
      warn(
