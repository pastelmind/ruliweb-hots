...
program
  ...
  .on('--help', () => {
    console.log('\nArguments:');
    console.log('  <json_dir>'.padEnd(30) + '  Directory containing JSON files generated by heroes-parser');
  })
  ...
  .action((jsonDir) => {
    if (jsonDir)
      program.jsonDir = path.resolve(jsonDir);
  })
  .parse(process.argv);
...
(async () => {
  ...
  for (const dataFile of dataFiles) {
    try {
      const heroDataSource = await readFileAsync(path.join(program.jsonDir, dataFile), 'utf8');
